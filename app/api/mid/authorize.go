...
func Authorize(ctx context.Context, log *logger.Logger, client *authclient.Client, rule string, handler Handler) Response {
	userID, err := GetUserID(ctx)
	if err != nil {
		return appError(errs.Unauthenticated, err)
	}
...
// user id.
func AuthorizeUser(ctx context.Context, log *logger.Logger, client *authclient.Client, userBus *userbus.Business, rule string, id string, handler Handler) Response {
	...
	if id != "" {
		...
		if err != nil {
			return appError(errs.Unauthenticated, ErrInvalidID)
		}
		...
		if err != nil {
			...
			case errors.Is(err, userbus.ErrNotFound):
				return appError(errs.Unauthenticated, err)
			default:
				return appErrorf(errs.Unauthenticated, "querybyid: userID[%s]: %s", userID, err)
			}
		}
...
func AuthorizeProduct(ctx context.Context, log *logger.Logger, client *authclient.Client, productBus *productbus.Business, id string, handler Handler) Response {
	var userID uuid.UUID
	...
	if id != "" {
		...
		if err != nil {
			...
			case errors.Is(err, productbus.ErrNotFound):
				return appError(errs.Unauthenticated, err)
			default:
				return appErrorf(errs.Internal, "querybyid: productID[%s]: %s", productID, err)
			}
		}
...
func AuthorizeHome(ctx context.Context, log *logger.Logger, client *authclient.Client, homeBus *homebus.Business, id string, handler Handler) Response {
	var userID uuid.UUID
	...
	if id != "" {
		...
		if err != nil {
			...
			case errors.Is(err, homebus.ErrNotFound):
				return appError(errs.Unauthenticated, err)
			default:
				return appErrorf(errs.Unauthenticated, "querybyid: homeID[%s]: %s", homeID, err)
			}
		}
