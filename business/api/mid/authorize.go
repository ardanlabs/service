	...
	m := func(handler web.Handler) web.Handler {
		h := func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
			...
			if id := web.Param(r, "user_id"); id != "" {
				...
				if err != nil {
					return errs.New(http.StatusUnauthorized, ErrInvalidID)
				}
				...
				if err != nil {
					...
					default:
						return errs.Newf(http.StatusUnauthorized, "querybyid: userID[%s]: %s", userID, err)
					}
				}
	...
	m := func(handler web.Handler) web.Handler {
		h := func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
			...
			if id := web.Param(r, "product_id"); id != "" {
				...
				if err != nil {
					...
					case errors.Is(err, product.ErrNotFound):
						return errs.New(http.StatusUnauthorized, err)
					default:
						return errs.Newf(http.StatusInternalServerError, "querybyid: productID[%s]: %s", productID, err)
					}
				}
			...
			if err := a.Authorize(ctx, claims, userID, auth.RuleAdminOrSubject); err != nil {
				return errs.Newf(http.StatusUnauthorized, "authorize: you are not authorized for that action, claims[%v] rule[%v]: %s", claims.Roles, auth.RuleAdminOrSubject, err)
			}
			...
			if id := web.Param(r, "home_id"); id != "" {
				...
				if err != nil {
					...
					case errors.Is(err, home.ErrNotFound):
						return errs.New(http.StatusUnauthorized, err)
					default:
						return errs.Newf(http.StatusUnauthorized, "querybyid: homeID[%s]: %s", homeID, err)
					}
				}
